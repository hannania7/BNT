...
class ReadObsInsertDB():
    ...
    def obs_db_insert(self):
        if self.station_name == 'hf':
            ...
            if self.check_obs_time(table_name):
                self.data_sql = f"insert into {table_name} {data_value_set} values %s"
            else:
                obs_post_id = self.obs_object.obs_data_list[0]
                obs_time = self.obs_object.obs_data_list[1]
                self.data_sql = f"update {table_name} set {data_value_set} = %s where obs_post_id='{obs_post_id}' and obs_time='{obs_time}';"

            with connect() as connection:
                with connection.cursor() as cursor:
                    cursor.execute(self.data_sql, (self.obs_object.obs_data_list, ))
                cursor.close()
            connection.close()
