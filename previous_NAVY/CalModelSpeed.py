...
def make_yes3k_speed_nc(ncp, output_path, speed_arr, direction_arr):
    ...
    temp = w_ncp.createVariable('temp', 'f', ('ocean_time', 'lat', 'lon'))
    speed = w_ncp.createVariable('speed', 'f', ('ocean_time', 'lat', 'lon'))
    direction = w_ncp.createVariable('direction', 'f', ('ocean_time', 'lat', 'lon'))
    mask_rho = w_ncp.createVariable('mask_rho', 'd', ('lat', 'lon'))
...
def make_wrf_speed_nc(ncp, output_path, speed_arr, direction_arr):
    ...
    swrad = w_ncp.createVariable('Swrad', 'f', ('ocean_time', 'lat', 'lon'))
    tair = w_ncp.createVariable('Tair', 'f', ('ocean_time', 'lat', 'lon'))
    pair = w_ncp.createVariable('Pair', 'f', ('ocean_time', 'lat', 'lon'))
    speed = w_ncp.createVariable('speed', 'f', ('ocean_time', 'lat', 'lon'))
    direction = w_ncp.createVariable('direction', 'f', ('ocean_time', 'lat', 'lon'))
...
def main_yes3k(col_head_row, date, output_dir_path):
    ...
    output_file_name = file_name.replace('regrid', 'speed')
    ...
    make_yes3k_speed_nc(ncp, output_path, speed_arr, direction_arr)
...
def main_wrf(col_head_row, date, output_dir_path):
    ...
    file_name = os.path.basename(input_path)
    output_file_name = file_name.replace('regrid', 'speed1')
    output_path = f"{output_dir_path}/{output_file_name}"
    ...
    make_wrf_speed_nc(ncp, output_path, speed_arr, direction_arr)
    # os.remove(input_path)
