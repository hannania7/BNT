...
def vprofile(filepath, varname, latitude, longitude, xrange, time):
    ...
    number_string = str(re.findall("_\d+", filepath)[2])
    ocean_time = datetime.datetime(year=int(number_string[1:5]), month=int(number_string[5:7]),
                                   day=int(number_string[7:9]), hour=int(number_string[9:11]))
    ...
    plt.savefig('/DATA/PYTHON/output/mohid2k/vprofile/png/vprofile_%s_%s_%s_%s,%s_%s,%s,%s_%02d.png' % \
    ...
    with open('/DATA/PYTHON/output/mohid2k/vprofile/txt/vprofile_%s_%s_%s_%s,%s_%s,%s,%s_%02d.txt' % \
              (foldername.lower(), ocean_time.strftime("%Y%m%d%H"), varname, latitude, longitude,
