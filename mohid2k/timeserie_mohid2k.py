...
def timeserie_mohid(filepath,varname, latitude, longitude, starttime, endtime, s_rho):
    ...
    # output 폴더가 없으면 생성
    if not os.path.exists('/DATA/PYTHON/output/mohid2k/timeserie/json'):
        os.makedirs('/DATA/PYTHON/output/mohid2k/timeserie/json')
    if not os.path.exists('/DATA/PYTHON/output/mohid2k/timeserie/txt'):
        os.makedirs('/DATA/PYTHON/output/mohid2k/timeserie/txt') 
    ...
    if os.path.exists(filepath): # if file exists
        ...
        with open('/DATA/PYTHON/output/mohid2k/timeserie/json/timeserie_%s_%s_%s_%s,%s_%s,%s_%s.json' % \
                  (foldername.lower(),yyyymmddhh,varname,latitude,longitude,starttime,endtime,s_rho), "w") as fj:
            json.dump(data,fj)

        with open('/DATA/PYTHON/output/mohid2k/timeserie/txt/timeserie_%s_%s_%s_%s,%s_%s,%s_%s.txt' % \
