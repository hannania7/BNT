...
def sshpass_obs(column_row, col_datetime):
    ...
    obs_time = col_datetime.strftime('%H%M')
    server_file_path = MakeFilePath.get_obs_remote_path(column_row, col_datetime)
    ...
    save_file_path = MakeFilePath.get_obs_save_file_path(column_row, col_datetime, save_dir_path)
    ...
    if col_stat == '1':
        ...
        server_remove_path = MakeFilePath.get_server_remove_log_path(col_datetime)
...
def	sshpass_pred(column_row, date):
        ...
        if data_cate2 == 'MOHID300M':
                if data_name == 'MOHID300M':
                        server_file_path = MakeFilePath.get_mohid300_remote_path(date)
                        save_file_path = MakeFilePath.get_mohid300_save_file_path(column_row, date)
                elif data_name == 'TCSMap':
                        server_file_path = MakeFilePath.get_mohid_tcs_remote_path(date)
                        save_file_path = MakeFilePath.get_mohid_tcs_save_file_path(column_row, date)

        elif data_cate2 == "YES3K":
                server_file_path = MakeFilePath.get_yes3k_remote_path(date)
                save_file_path = MakeFilePath.get_yes3k_save_file_path(column_row, date)

        elif data_cate2 == "WRF":
                server_file_path = MakeFilePath.get_wrf_remote_path(date, data_name.lower())
                save_file_path = MakeFilePath.get_wrf_save_file_path(column_row, date)

        elif data_cate2 == "WW3":
                server_file_path = MakeFilePath.get_ww3_remote_path(date)
                save_file_path = MakeFilePath.get_ww3_save_file_path(column_row, date)

        elif data_cate2 == 'SATELLITE':
                if data_name == "RGB":
                        server_file_path = MakeFilePath.get_rgb_remote_path(date)
                        save_file_path = MakeFilePath.get_rgb_save_file_path(column_row, date)

                elif data_name == "DETECT":
                        pass

        elif data_cate2 == 'sf':
                server_file_path = MakeFilePath.get_sf_remote_path(column_row, date)
                save_file_path = MakeFilePath.get_sf_save_file_path(column_row, date)

        elif data_cate2 == "MID":
                server_file_path = MakeFilePath.get_mid_remote_path(date)
                save_file_path = MakeFilePath.get_mid_save_path(column_row, date)
        ...
        if subpro_check[1] == 'File exists':
                # 2022.07.21 원태찬 test하기 위해 주석처리함
                #subprocess.run(f'sshpass -p {server_pw} scp -P 22 -o "StrictHostKeyChecking=no" -r {server}:{server_file_path} {save_dir_path}', shell=True)
                pass
        # 대내연동수신서버에 파일이 없을 경우
        ...
        if data_cate2 == 'SATELLITE':
                for i in range(12):
                        file_num = "%03d" % i
                        if data_name == "RGB":
                                tile_file_path = MakeFilePath.get_rgb_tile_file_path(column_row, date, file_num)
                        elif data_name == "DETECT":
                                tile_file_path = None
                        if tile_file_path:
...
def sshpass_anal(column_row, date):
    ...
    data_name = column_row.get('data_name')
    if data_cate2 == 'MOHID300M':
        save_dir_path = MakeFilePath.get_anal_vis_save_dir_path(column_row, date)
        server_file_path = MakeFilePath.get_anal_vis_remote_file_path(column_row, date)
        save_file_path = MakeFilePath.get_anal_vis_save_file_path(column_row, date)

    elif data_cate2 == "YES3K":
        save_dir_path = MakeFilePath.get_anal_yes3k_save_dir_path(column_row, date)
        server_file_path = MakeFilePath.get_anal_yes3k_remote_file_path(column_row)
        save_file_path = MakeFilePath.get_anal_yes3k_save_file_path(column_row, date)
    else:
        save_dir_path = column_row.get('data_path')
    ...
    if col_stat == '1':
        subprocess.run(f'sshpass -p {server_pw} ssh -o "StrictHostKeyChecking=no" {server} rm -f {server_file_path}', shell=True)
        server_remove_path = MakeFilePath.get_server_remove_log_path(date)
        remove_log = get_remove_log(server_file_path, save_file_path)
        write_log(server_remove_path, remove_log)
    return col_stat
...
def check_obs_file(column_row, file_path, obs_date, time):
		...
		obs_data = obs_date + time
		col_data_time = datetime.datetime.strptime(obs_data, "%Y%m%d%H%M")
		col_data_time_str = col_data_time.strftime('%Y/%m/%d %H:%M:%S')
		print(col_data_time_str)
		if os.path.isfile(file_path):
...
def manual_col_main(data_no, col_date, col_time):
    ...
    if data_cate1 == 'obs':
        ...
        obs_time = "00:00"
        pred_time = PreMain.get_model_pred_time(col_head_row, col_datetime)
    ...
    if col_stat == '1':
        if data_cate1 == 'obs':
            ...
            obs_date = col_datetime.strftime("%Y%m%d%H%M")
            ObsMain_obs.main(obs_date, station, data_name)
		# 예측자료와 대내연동수신서버에 파일이 있으면,
        elif data_cate1 == 'pred' and subpro_check[1] == 'File exists':
            PreMain.main(col_head_row, col_datetime)
            # 2022.07.21 원태찬 test하기 위해 주석처리함 
            #subprocess.run(f'sshpass -p {server_pw} ssh -o "StrictHostKeyChecking=no" {server} rm -f {server_file_path}', shell=True)
            server_remove_path = MakeFilePath.get_server_remove_log_path(col_datetime) 
            remove_log = get_remove_log(server_file_path, save_file_path)
    ...
    log_path = MakeFilePath.get_log_path(col_head_row, col_date)
...
if __name__ == "__main__":
    ...
    #manual_col_main(102, '20220302','3')
