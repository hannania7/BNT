...
def main_yes3k(col_head_row, date, output_dir_path):
    ...
    if data_name == 'YES3K':
        ...
        input_path = MakeFilePath.get_yes3k_regrid_file_8D_path(col_head_row, date)
    file_name = os.path.basename(input_path) 
    if (data_name == 'YES3K' and model == 'YES3K'):
        output_file_name = file_name.replace('2_regrid', 'speed')
    elif (data_name == 'YES3K_8D' and model == 'YES3K_8D'):
        output_file_name = file_name.replace('2_regrid', 'speed')
    ...
    # 7일
        ...
        yes3k_save_file_path = f"{data_path}/YES3K_{model_date}_8D_2.nc"
...
def main_wrf(col_head_row, date, output_dir_path):
    ...
    if data_name == 'DM1' or data_name == 'DM2':
        ...
        input_path = MakeFilePath.get_wrf_regrid_file_8D_path(col_head_row, date)
    file_name = os.path.basename(input_path)      
    if model == 'WRF':
        output_file_name = file_name.replace('2_regrid', 'speed')
    elif model == 'WRF_8D' or model == 'WRF_8D_2':
        output_file_name = file_name.replace('2_regrid', 'speed')
    output_path = f"{output_dir_path}/{output_file_name}"
    ...
    if model == 'WRF':
        ...
        wrfdm_save_file_path = f"{data_path}/wrf{data_name}_{start_date}_{end_date}_2.nc"
            ...
            wrfdm_save_file_path = f"{data_path}/wrf{data}_{start_date}_{end_date}_8D_2.nc"
    elif model == 'WRF_8D_2':
            data = 'dm2'
            end_date = (date + datetime.timedelta(days=6)).strftime("%Y%m%d")
            wrfdm_save_file_path = f"{data_path}/wrf{data}_{start_date}_{end_date}_8D_2.nc"
