...
class ObsObject:
		...
		def read_obs_data(self, station_path, station, data_name, obs_date):
				know_sql_filepath = f"select data_name from mng_data_col_head where data_cate2 = '{station}' and data_name = '{data_name}'"
				know_filepath = read_query(know_sql_filepath)
				folder_path = get_station_folder_path(station, know_filepath['data_name'][0])
				if self.station_name == 'ie' or self.station_name == 'kg':
					know_filepath['data_name'][0] = know_filepath['data_name'][0].upper()
				else:
					pass
				# print('a')
				if os.path.isfile(f"{folder_path}/{know_filepath['data_name'][0]}{self.obs_date[0:8]}.dat"):
					with open(f"{folder_path}/{know_filepath['data_name'][0]}{self.obs_date[0:8]}.dat", 'r', encoding='ISO-8859-1') as fp:
						know_index_length = list()
						read_obs_data_row_len = fp.readlines()[4:]
						for index, data in enumerate(read_obs_data_row_len):
							know_index_length.append(index)
						print(know_index_length[-1])
						if self.station_name == 'ie':
							table_name = self.station_name + '_station_data'
							table_name = table_name.replace('data', know_filepath['data_name'][0].lower())
							num_obs_post_id = f"select * from {table_name} where obs_post_id = '{know_filepath['data_name'][0].lower()}' order by obs_time desc LIMIT 1;"
						elif self.station_name == 'kg':
							table_name = self.station_name + '_station_data'
							num_obs_post_id = f"select * from {table_name} where obs_post_id = '{know_filepath['data_name'][0].lower()}' order by obs_time desc LIMIT 1;"	
						else:
							table_name = self.station_name + '_station_data'
							num_obs_post_id = f"select * from {table_name} where obs_post_id = '{know_filepath['data_name'][0]}' order by obs_time desc LIMIT 1;"
						# 제일 최근 거 1개만 limit
						check_num = read_query(num_obs_post_id)
						# print(len(check_num['obs_time']))
