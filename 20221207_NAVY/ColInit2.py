...
def	sshpass_pred(column_row, date):
    ...
    # 8일				
    elif data_cate2 == "YES3K_8D" and data_name == 'YES3K_8D': 
        server_file_path = MakeFilePath2.make_yes3k_remote_path_8D(date)
        save_file_path = MakeFilePath2.make_yes3k_save_file_path_8D(column_row, date)
                    
    elif data_cate2 == "WRF_8D" and data_name == 'DM1_8D': 
        server_file_path = MakeFilePath2.make_wrf_remote_path_8D(date, data_name.lower())
        save_file_path = MakeFilePath2.make_wrf_save_file_path_8D(column_row, date) 

    elif data_cate2 == "WRF_8D_2" and data_name == 'DM2_8D': 
        server_file_path = MakeFilePath2.make_wrf_remote_path_8D(date, data_name.lower())
        save_file_path = MakeFilePath2.make_wrf_save_file_path_8D(column_row, date) 
    ...
    if data_cate2 == 'SATELLITE':
        for i in range(12):
            ...
            if data_name == "RGB":
                ...
                tile_file_path = None
            if tile_file_path:
                subprocess.run(f'sshpass -p {server_pw} scp -P 22 -o "StrictHostKeyChecking=no" -r {server}:{tile_file_path} {save_dir_path}', shell=True)
...
def manual_col_main(data_no, col_date, col_time):
    ...
    now = str(now_date.strftime("%Y%m%d"))
    print(now)
    ...
    if len(col_log_row) > 0:
        ...
        elif (data_cate1 == 'obs' or data_cate1 == 'anal'):
            sql_set = f"col_stat='{col_stat}', col_cnt='{col_cnt}', reg_date='{now_date}'"
            sql_where = log_sql.split('where')[1]
            DBInsertModule.log_db_update(sql_set, sql_where)
    ...
    if col_stat == '1':
        if data_cate1 == 'obs':
            ...
            ObsMain_new.main(obs_date, station, data_name)
