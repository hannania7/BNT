...
# ColManual 7일 예측자료
def make_yes3k_remote_path_8D(date):
    remote_dir_path = PROPERTY['REMOTE_PATH']
    model_date = date.strftime("%Y%m%d00")
    remote_file_path = f"{remote_dir_path}/YES3K_8D_{model_date}.nc"
    return remote_file_path


def make_yes3k_save_file_path_8D(column_row, date):
    data_path = get_pred_save_dir_path(column_row, date)
    model_date = date.strftime("%Y%m%d00")
    save_file_path = f"{data_path}/YES3K_8D_{model_date}.nc"
    return save_file_path
...
def change_coordinate_path_8D(column_row, date, model):
    if model == 'YES3K':
        data_path = get_pred_save_dir_path(column_row, date)
        model_date = date.strftime("%Y%m%d00")
        save_file_path = f"{data_path}/{model}_{model_date}_8D_2.nc"
        ...
        save_file_path = f"{data_path}/wrf{data}_{start_date}_{end_date}_8D_2.nc"
    return save_file_path
...
def get_yes3k_regrid_file_8D_path(column_row, date):
    data_path = get_pred_save_dir_path(column_row, date)
    model_date = date.strftime("%Y%m%d00")
    save_file_path = f"{data_path}/YES3K_{model_date}_8D_2_regrid.nc"
    return save_file_path
...
def get_wrf_remote_path(date, data_name):
    ...
    elif data_name == 'dm1_8d' or data_name == 'dm2_8d':
        if data_name == 'dm1_8d':
            data = 'dm1'
        elif data_name == 'dm2_8d':
            data = 'dm2'
...
def make_wrf_save_file_path_8D(column_row, date):
    data_name = column_row.get('data_name').lower()
    if data_name == 'dm1_8d':
        data = 'dm1'
    elif data_name == 'dm2_8d':
...
def make_wrf_remote_path_8D(date, data_name):
		if data_name == 'dm1_8d':
				data = 'dm1'
		elif data_name == 'dm2_8d':
				data = 'dm2'
...
# 2022.6.22 원태찬 7일치 예측자료 추가
def get_wrf_regrid_file_8D_path(column_row, date):
    data_name = column_row.get('data_name').lower()
    if data_name == 'dm1_8D':
        data = 'dm1'
    elif data_name == 'dm2_8D':
    ...
    start_date = date.strftime("%Y%m%d")
    end_date = (date + datetime.timedelta(days=7)).strftime("%Y%m%d")
    save_file_path = f"{data_path}/wrf{data}_{start_date}_{end_date}_8D_2_regrid.nc"
    return save_file_path
