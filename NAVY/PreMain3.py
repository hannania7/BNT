...
import ParsingData
import ColManual2    
import CalModelSpeed3 
import netCDF4   
# from datetime import datetime
import pandas as pd
import pyproj    
import MakeFilePath3
...
def run_model_regrid(input_file_path:str, weight_file_path, ncl_path, date) -> str:
    ...
    if ext == '.nc':
        output_file_path = MakeFilePath3.get_regrid_path(input_file_path)
    ...
    if model == 'sf':
        ...
        if model == 'mohid300':
            ...
            if (data_name == 'yes3k' and model == 'yes3k') or (model == 'wrfdm1') or (model == 'wrfdm2') or (model == 'ww3'):
                ...
                # else:
                #     flag = True
                #     st_id = DBInsertModule.get_already_st_id(model, table_date)
                #     c_st_id = st_id + time_val
                #     dataset_sql = f"update model_{model}_dataset_{table_date} set (st_id, create_time, pred_time, receive_time) = %s where st_id = {c_st_id}"
                #     model_object.model_dataset_update(dataset_sql, c_st_id)
                #     data_sql = f"update model_{model}_data_{table_date} set {input_column} = %s where st_id = {c_st_id}"
                #     model_object.model_data_update(data_sql, c_st_id)
            # 7일
            elif (data_name == 'yes3k_7d' and model == 'yes3k_7d') or (model == 'wrfdm1_7d'):
                for time_val in range(168):
                    if model == 'yes3k':
                        model_object = DBInsertModule.ReadYes3kInsertDB(file_path)
                        input_column = "(st_id, latitude, longitude, u, v, wtemp, salt, cspeed, cdir, idx_x, idx_y)"
                    ...
                    if check_dbinsert(f'model_{model}_dataset', model_object):
                        flag = True
                        DBInsertModule.get_st_id(model_object, model)
                        dataset_sql = f"insert into model_{model}_7d_dataset_{table_date} (st_id, create_time, pred_time, receive_time) values %s"
                        model_object.model_dataset_insert(dataset_sql)
                        data_sql = f"insert into model_{model}_7d_data_{table_date} {input_column} values %s"
                        model_object.model_data_insert(data_sql)
 
            # if flag and model == 'yes3k' or (model == 'wrfdm1'):
            #     CalModelSpeed3.main(model.upper(), table_date)
...
def scp_sar(file_path, input_dir, date):
    ...
    server_scp_path = MakeFilePath3.get_sar_scp_log_path(date)
    scp_log = ColManual2.get_scp_log(file_path, sar_path)
    ColManual2.write_log(server_scp_path, scp_log)

def scp_sar_7D(file_path, file_out, input_dir, date):
    sar_ip = PROPERTY["SAR"]
    sar_pw = PROPERTY["SAR_PW"]
...
def change_coordinate(path, data_row, date, model, data_name):
    ...
    if (data_name == 'YES3K' and model == 'YES3K') or (data_name == 'YES3K_7D' and model == 'YES3K'):
        ...
        if (data_name == 'YES3K_7D' and model == 'YES3K'):
        ...
        if (data_name == 'YES3K' and model == 'YES3K'):
            change_file_path = MakeFilePath3.change_coordinate_path(data_row, date, model)
        # 7일
        elif (data_name == 'YES3K_7D' and model == 'YES3K'):
            change_file_path = MakeFilePath3.change_coordinate_path_7D(data_row, date, model)
    ...
    elif (model == 'WRFDM1') or (model == 'WRFDM1_7D') or (model == 'WRFDM2') or (model == 'WRFDM2_7D'):
        ncp = netCDF4.Dataset(path, 'r')
        lon_rho = ncp.variables['lon_rho'][:]
        lat_rho = ncp.variables['lat_rho'][:]
        r_ocean_time = ncp.variables['ocean_time'][:]
        ...
        lon_rho = result[:, 0]
        lat_rho = result[:, 1]
        if model == 'WRFDM1' or model == 'WRFDM1_7D':
            r_lon_rho = lon_rho.reshape(217,183)
            r_lat_rho = lat_rho.reshape(217,183)
        elif model == 'WRFDM2' or model == 'WRFDM2_7D':
...
def main(data_row, date):
        ...
        ncl_file_path_7D = f'{PROPERTY["NCL_PATH"]}/regrid_{model2.lower()}_7D.ncl'
        weight_file_path = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_{model.lower()}.nc'
        weight_file_path_7D = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_{model2.lower()}.nc'

        if model == 'MOHID300M':
                model = 'MOHID300'
                mohid300m_file_path = MakeFilePath3.get_mohid300_save_file_path(data_row, date)
                if os.path.isfile(mohid300m_file_path):
                        model_db_insert(mohid300m_file_path, model, date, data_name)

        elif data_name == 'TCSMap':
                        mohid_tcs_file_path = MakeFilePath3.get_mohid_tcs_save_file_path(data_row, date)
                        if os.path.isfile(mohid_tcs_file_path):
                                scp_sar(mohid_tcs_file_path, 'L4_OC', date)
        # 3일치
        elif (data_name == 'YES3K' and model == 'YES3K'):
                yes3k_file_path = MakeFilePath3.get_yes3k_save_file_path(data_row, date)
                if os.path.isfile(yes3k_file_path):
                        ...
                        CalModelSpeed3.main(model.upper(), table_date, data_name)
        # 7일치
        elif (data_name == 'YES3K_7D' and model == 'YES3K_7D'):
                yes3k_file_path = MakeFilePath3.make_yes3k_save_file_path_7D(data_row, date)
                if os.path.isfile(yes3k_file_path):  
                        base_yes3k_file_path = os.path.basename(yes3k_file_path)
                        yes3k_file_path_tr = base_yes3k_file_path.replace('_7D','')
                        scp_sar_7D(yes3k_file_path, yes3k_file_path_tr, 'YES3K_7D', date)
                        # yes3k_regrid_path = run_model_regrid(yes3k_file_path, weight_file_path_7D, ncl_file_path_7D, date)
                        # model_db_insert(yes3k_regrid_path, model, date, data_name)
                        # yes3k_file_path_2 = change_coordinate(yes3k_file_path, data_row, date, model, data_name)
                        # yes3k_regrid_path_2 = run_model_regrid(yes3k_file_path_2, weight_file_path_7D, ncl_file_path_7D, date)
                        # table_date = date.strftime('%Y%m%d')
                        # CalModelSpeed3.main(model.upper(), table_date, data_name)

        elif model == 'WW3':
                ww3_file_path = MakeFilePath3.get_ww3_save_file_path(data_row, date)
        ...
        # 3일치
        elif (data_name == 'DM1') and (model == 'WRFDM1'):
                model = 'WRF' + data_name
                weight_file_path = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_{model.lower()}.nc'
                if data_name == 'DM1':
                        wrfdm1_file_path = MakeFilePath3.get_wrf_save_file_path(data_row, date)
        ...
        elif (data_name == 'DM2') and (model == 'WRFDM2'):
                model = 'WRF' + data_name
                weight_file_path = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_{model.lower()}.nc'
                if data_name == 'DM2':
                        wrfdm2_file_path = MakeFilePath3.get_wrf_save_file_path(data_row, date)
        ...
        # 7일치
        elif (data_name == 'DM1_7D') and (model == 'WRF_7D'):
                model = 'WRF' + data_name
                weight_file_path = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_wrfdm1.nc'
                if data_name == 'DM1_7D':
                        wrfdm1_file_path = MakeFilePath3.make_wrf_save_file_path_7D(data_row, date)
                        if os.path.isfile(wrfdm1_file_path):
                                base_wrfdm1_file_path = os.path.basename(wrfdm1_file_path)
                                scp_sar_7D(wrfdm1_file_path, base_wrfdm1_file_path, 'WRF_7D', date)
                                # wrfdm1_regrid_path = run_model_regrid(wrfdm1_file_path, weight_file_path_7D, ncl_file_path_7D, date)
                                # model_db_insert(wrfdm1_regrid_path, model, date, data_name)
                                # wrfdm1_file_path_2 = change_coordinate(wrfdm1_file_path, data_row, date, model, data_name)
                                # wrfdm1_regrid_path_2 = run_model_regrid(wrfdm1_file_path_2, weight_file_path_7D, ncl_file_path_7D, date)
                                # table_date = date.strftime('%Y%m%d')
                                # CalModelSpeed3.main(model.upper(), table_date, data_name)

        elif (data_name == 'DM2_7D') and (model == 'WRF_7D_2'):
                model = 'WRF' + data_name
                weight_file_path = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_wrfdm2.nc'
                if data_name == 'DM2_7D':
                        wrfdm2_file_path = MakeFilePath3.make_wrf_save_file_path_7D(data_row, date)
                        if os.path.isfile(wrfdm2_file_path):
                                base_wrfdm2_file_path = os.path.basename(wrfdm2_file_path)
                                scp_sar_7D(wrfdm2_file_path, base_wrfdm2_file_path, 'WRF_7D', date)
                                # wrfdm1_regrid_path = run_model_regrid(wrfdm2_file_path, weight_file_path_7D, ncl_file_path_7D, date)
                                # model_db_insert(wrfdm2_regrid_path, model, date, data_name)
                                # wrfdm2_file_path_2 = change_coordinate(wrfdm2_file_path, data_row, date, model, data_name)
                                # wrfdm2_regrid_path_2 = run_model_regrid(wrfdm2_file_path_2, weight_file_path_7D, ncl_file_path_7D, date)
                                # table_date = date.strftime('%Y%m%d')
                                # CalModelSpeed3.main(model.upper(), table_date, data_name)

        elif model == 'sf':
                input_file_path = MakeFilePath3.get_sf_save_file_path(data_row, date)
                if os.path.isfile(input_file_path):
                        model_db_insert(input_file_path, model, date, data_name)

        elif model == 'SATELLITE':
                input_file_path = MakeFilePath3.get_rgb_save_file_path(data_row, date)
                if os.path.isfile(input_file_path):
                        model_db_insert(input_file_path, model, date, data_name)

        elif model == 'MID':
                input_file_path = MakeFilePath3.get_mid_save_path(data_row, date)
                if os.path.isfile(input_file_path):
