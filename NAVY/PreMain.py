...
def model_db_insert(file_path, model, model_date, data_name):
    if model == 'sf':
        ...
        if model == 'mohid300':
            ...
            # 7일
            elif (data_name == 'yes3k_7d' and model == 'yes3k') or (model == 'wrfdm1_7d'):
...
def change_coordinate(path, data_row, date, model, data_name):
    ...
    if (data_name == 'YES3K' and model == 'YES3K') or (data_name == 'YES3K_7D' and model == 'YES3K'):
        ...
        if (data_name == 'YES3K' and model == 'YES3K'):
            change_file_path = MakeFilePath.change_coordinate_path(data_row, date, model)
        # 7일
        elif (data_name == 'YES3K_7D' and model == 'YES3K'):
            change_file_path = MakeFilePath.change_coordinate_path_7D(data_row, date, model)
...
def main(data_row, date):
    ...
    # 7일치
        ...
        yes3k_file_path = MakeFilePath.get_yes3k_save_file_path_7D(data_row, date)
        if os.path.isfile(yes3k_file_path): 
            scp_sar(yes3k_file_path, 'YES3K', date)
    ...
    # 7일치
    elif (data_name == 'DM1_7D') and (model == 'WRF'):
        model = model + data_name
        weight_file_path = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_wrfdm1.nc'
        if data_name == 'DM1_7D':
            wrfdm1_file_path = MakeFilePath.get_wrf_save_file_path_7D(data_row, date)
            if os.path.isfile(wrfdm1_file_path):
                scp_sar(wrfdm1_file_path, 'WRF', date)
    ...
    elif (data_name == 'DM2_7D') and (model == 'WRF'):
        model = model + data_name
