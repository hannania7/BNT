...
def change_coordinate(path, data_row, date, model, data_name):
    ...
    if (data_name == 'YES3K' and model == 'YES3K') or (data_name == 'YES3K_7D' and model == 'YES3K'):
        ...
        # 7일
            ...
            change_file_path = MakeFilePath2.change_coordinate_path_7D(data_row, date, model)
...
def main(data_row, date):
        ...
        # 7일치
                ...
                yes3k_file_path = MakeFilePath2.make_yes3k_save_file_path_7D(data_row, date)
                if os.path.isfile(yes3k_file_path):  
        ...
        # 3일치
        elif (data_name == 'DM1') and (model == 'WRF'):
                model = 'WRF' + data_name
                weight_file_path = f'{PROPERTY["WEIGHT_PATH"]}/wgt_file_{model.lower()}.nc'
                if data_name == 'DM1':
        ...
        # 7일치
                ...
                if data_name == 'DM1_7D':
                        wrfdm1_file_path = MakeFilePath2.make_wrf_save_file_path_7D(data_row, date)
                        if os.path.isfile(wrfdm1_file_path):
                                start_date = date.strftime("%Y%m%d")
                                end_date = (date + datetime.timedelta(days=2)).strftime("%Y%m%d")
                                base_save_file_path = f"wrfdm1_{start_date}_{end_date}.nc"
                                scp_sar_7D(wrfdm1_file_path, base_save_file_path, 'WRF_7D', date)
                ...
                if data_name == 'DM2_7D':
                        wrfdm2_file_path = MakeFilePath2.make_wrf_save_file_path_7D(data_row, date)
                        if os.path.isfile(wrfdm2_file_path):
                                start_date = date.strftime("%Y%m%d")
                                end_date = (date + datetime.timedelta(days=2)).strftime("%Y%m%d")
                                base_save_file_path = f"wrfdm2_{start_date}_{end_date}.nc"
                                scp_sar_7D(wrfdm2_file_path, base_save_file_path, 'WRF_7D', date)
